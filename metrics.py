...
class EvaluateCodesCallBack(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(step_size):
            batch_codes_x, batch_visit_lens, batch_note_x, batch_note_lens, batch_patient_gender, batch_patient_age, batch_patient_cluster = self.data_gen[i]
            output = self.model(inputs={
                'visit_codes': batch_codes_x,
                'visit_lens': batch_visit_lens,
                'word_ids': batch_note_x,
                'word_lens': batch_note_lens,
                'patient_gender': batch_patient_gender,
                'patient_age': batch_patient_age,
                'patient_cluster': batch_patient_cluster
            }, training=False)

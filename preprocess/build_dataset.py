...
def sample_patients(patient_admission: dict, admission_codes: dict, sample_num: int, seed=8888) -> (dict, dict):
    print('select sample patients...')
    np.random.seed(seed)
    keys = list(patient_admission.keys())
    selected_pids = np.random.choice(keys, sample_num, False)
    new_patient_admission = {pid: patient_admission[pid] for pid in selected_pids}
    new_admission_codes = {}
    for admissions in new_patient_admission.values():
        for admission in admissions:
            adm_id = admission['admission_id']
            new_admission_codes[adm_id] = admission_codes[adm_id]
    return new_patient_admission, new_admission_codes


def split_patients4(patient_admission: dict, admission_codes: dict, code_map: dict, seed=8888) -> (
        np.ndarray, np.ndarray, np.ndarray):
    print('splitting train, valid, and test pids')
    np.random.seed(seed)
    common_pids = set()
...
def build_disease_y(hf_prefix: str, codes_y: np.ndarray, code_map: dict) -> np.ndarray:
    print('building train/valid/test diseases labels ...')
...
def build_patient_data(pids: np.ndarray, patient_gender: dict, patient_age: dict, patient_cluster: dict) -> (
        np.ndarray, np.ndarray, np.ndarray):
    print('building train/valid/test patient features ...')
    n = len(pids)
    ...
    for i, pid in enumerate(pids):
        print('\r\t%d / %d' % (i + 1, len(pids)), end='')
        x = patient_gender[pid]
        y = patient_age[pid]
        z = patient_cluster[pid]
        gender[i] = x
        age[i] = y
        cluster[i] = z
    print('\r\t%d / %d' % (len(pids), len(pids)))
    return gender, age, cluster

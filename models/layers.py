...
class CategoryEmbedding(Layer):
    def __init__(self, code_category, category_num, category_dim, name='category_embedding'):
        super().__init__(name=name)
        self.code_category = code_category
        self.category_embedding_layer = tf.keras.layers.Embedding(input_dim=category_num, output_dim=category_dim)

    def call(self, inputs=None):
        category_embeddings = self.category_embedding_layer(self.code_category)
...
class PatientFeatureEmbedding(Layer):
    def __init__(self, patient_gender_dim, patient_age_dim, patient_cluster_dim, name='patient_feature_embedding'):
        super().__init__(name=name)
        self.embedding_layer = tf.keras.layers.Embedding(input_dim=2, output_dim=patient_gender_dim)
        self.embedding_layer2 = tf.keras.layers.Embedding(input_dim=11, output_dim=patient_age_dim)
        self.embedding_layer3 = tf.keras.layers.Embedding(input_dim=10, output_dim=patient_cluster_dim)

    def call(self, patient_gender, patient_age, patient_cluster):
        embeddings = self.embedding_layer(patient_gender)
        embeddings2 = self.embedding_layer2(patient_age)
        embeddings3 = self.embedding_layer3(patient_cluster)
        patient_feature_embeddings = Concatenate()([embeddings, embeddings2, embeddings3])
        return patient_feature_embeddings


class SoftAttention(Layer):
    def __init__(self, hidden_size, name='soft_attention'):
        super().__init__(name=name)
        self.q = tf.keras.layers.Dense(1)
        self.W_1 = tf.keras.layers.Dense(hidden_size)

    def call(self, x):
        alpha = self.q(tf.sigmoid(self.W_1(x)))
        new_embedding = alpha * x
...
class GraphConvolution(Layer):
    def __init__(self, patient_dim, code_dim,
        ...
        self.c2p_denses = [Dense(dim, activation='softmax', name='code_to_patient_dense_%d' % layer)
                           for layer, dim in enumerate(c2p_dims)]
        self.p2c_denses = [Dense(dim*2, activation='softmax', name='patient_to_code_dense_%d' % layer)
                           for layer, dim in enumerate(p2c_dims)]
        self.p2c_denses2 = [Dense(dim, activation='softmax', name='patient_to_code_dense_%d' % layer)
                           for layer, dim in enumerate(p2c_dims)]
    ...
    def call(self, patient_embeddings, code_embeddings):
        ...
        patient_embeddings_c = self.p2c_denses2[-1](patient_embeddings)
...
class VisitEmbedding(Layer):
    ...
    def call(self, code_embeddings, visit_codes, visit_lens):
        ...
        visit_codes_num = tf.expand_dims(tf.reduce_sum(tf.cast(visit_codes > 0, visit_codes_embedding.dtype), axis=-1),
                                         axis=-1)
        visits_embeddings = tf.math.divide_no_nan(tf.reduce_sum(visit_codes_embedding, axis=-2), visit_codes_num)
        visit_mask = tf.expand_dims(tf.sequence_mask(visit_lens, self.max_seq_len, dtype=visits_embeddings.dtype),
                                    axis=-1)
...
class Attention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.w_omega = self.add_weight(name=self.name + '_w', shape=(hidden_size, self.attention_dim),
                                       initializer=GlorotUniform())
...
class TemporalEmbedding(Layer):
    ...
    def call(self, embeddings, lens):
        ...
        outputs = self.rnn_layers(embeddings) * tf.expand_dims(seq_mask,
                                                               axis=-1)  # (batch_size, max_seq_len, rnn_dim[-1])
